...
def test_init_params_random(land_model, dummy_data):
...
def test_init_params_mean(land_model, dummy_data):
...
@patch("land.exp_map")
@patch("land.log_map")
def test_fit(mock_log_map, mock_exp_map, mock_metric, mock_compute_norm, land_model, dummy_data):
    """Test the fit wrapper runs correctly and stops when loss diff < epsilon."""
    # Setup mocks
    mock_log_map.return_value = torch.tensor([1.0, 0.0])
    mock_exp_map.return_value = torch.zeros(2)
    mock_metric.return_value = torch.eye(2)
    ...
    # We will override loss function so that the loss_diff immediately falls below epsilon
    # which lets the loop terminate after 1 iteration.
    land_model.loss = MagicMock(side_effect=[torch.tensor(2.0), torch.tensor(2.0 + 1e-5)])
